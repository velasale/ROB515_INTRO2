...
class ArmIK:
    ...
    def transformAngelAdaptArm(self, theta3, theta4, theta5, theta6):
        #Convert the angle calculated by inverse kinematics into the corresponding pulse width value of the servo
        ...
        if servo3 > self.servo3Range[1] or servo3 < self.servo3Range[0] + 60:
            logger.info('servo3(%s) is out of range (%s, %s)', servo3, self.servo3Range[0] + 60, self.servo3Range[1])
            return False
        ...
        if servo4 > self.servo4Range[1] or servo4 < self.servo4Range[0]:
            logger.info('servo4(%s) is out of range (%s, %s)', servo4, self.servo4Range[0], self.servo4Range[1])
            return False
        ...
        if servo5 > ((self.servo5Range[1] + self.servo5Range[0])/2 + 90*self.servo5Param) or servo5 < ((self.servo5Range[1] + self.servo5Range[0])/2 - 90*self.servo5Param):
            logger.info('servo5(%s) is out of range (%s, %s)', servo5, self.servo5Range[0], self.servo5Range[1])
            return False
        ...
        if servo6 > self.servo6Range[1] or servo6 < self.servo6Range[0]:
            logger.info('servo6(%s) is out of range(%s, %s)', servo6, self.servo6Range[0], self.servo6Range[1])
            return False
